...
class Reader():
    def join_trade_registers(self, trade_register_path: str, csv_data_path: str):
        if not os.path.exists(trade_register_path):
            msg = f"trade register path txt file does not exist {trade_register_path}"
            error(msg)
        ...
        # contains_dash = processed_rtf["Year(s) Weapon of Order"].str.contains('-').unique()

        # info(f"dash count is: {contains_dash}")
        # missing_count = processed_rtf['No. Comments'].is_null().sum()
        # info(f"missing comments: {missing_count}")

        csv_df = self.read_csv_data(csv_data_path)

        joined_df = self.joined_table(processed_rtf, csv_df)
        joined_df.write_csv("joined_data_test.csv")
        
        info("joined data frames and wrote if to 'joined_data_test.csv' ")
    ...
    def create_trade_register_df(self, trade_register_path: str) -> pl.DataFrame:        
        ...
        # This needs to find the file in the folder as a normal rtf with no special name
        ...
        processed_rtf.write_csv("processed_rtf_test.csv")
        info(" wrote processed_rtf_test")
